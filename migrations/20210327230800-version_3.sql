ALTER TABLE `customers`
    ADD `pass_serial` VARCHAR(5)  NULL DEFAULT NULL COMMENT 'серия паспорта' AFTER `gender`,
    ADD `pass_number` VARCHAR(10) NULL DEFAULT NULL COMMENT 'номер паспорта' AFTER `pass_serial`;
...
ALTER TABLE `tickets`
    ...
    ADD `action`                ENUM ('surcharge','code-1','code-2','code-3')           NULL COMMENT 'выбор пользователя' AFTER `amount`,
...
ALTER TABLE `consents`
    ...
    ADD `signed_pass_serial`  VARCHAR(5)                NULL     DEFAULT NULL COMMENT 'серия паспорта' AFTER `signed_patronimic`,
    ADD `signed_pass_number`  VARCHAR(10)               NULL     DEFAULT NULL COMMENT 'номер паспорта' AFTER `signed_pass_serial`,
